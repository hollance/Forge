 ...
 */
class Inception3Net: NeuralNetwork {
  ...
  required public init(device: MTLDevice, inflightBuffers: Int) {
    ...
    for _ in 0..<inflightBuffers {
      sftImage.append(MPSImage(device: device, imageDescriptor: sftid))
    }
  ...
  public func encode(commandBuffer: MTLCommandBuffer, texture sourceTexture: MTLTexture, inflightIndex: Int) {
    ...
    logits_layer(commandBuffer: commandBuffer, inflightIndex: inflightIndex)
  ...
  func logits_layer(commandBuffer: MTLCommandBuffer, inflightIndex: Int) {
    // These images are only needed in this layer and will not be read by the CPU or
    // outside of the command bufer, so we can allocate them as MPSTemporaryImages and
    // save the CPU cost and memory size of allocating reserved storage for them.
    //
    ...
    softmax.encode(commandBuffer: commandBuffer, sourceImage: fc0Image, destinationImage: sftImage[inflightIndex])

  }

  public func fetchResult(inflightIndex: Int) -> NeuralNetworkResult {
    let probabilities = sftImage[inflightIndex].toFloatArray()
    ...
    //result.debugTexture = srcImage.texture
    return result
  }
